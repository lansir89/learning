...
def index(request):
    prefix = ''.join([
        'm' if request.mobile else 'p',         #request.mobile在中间件中定义
    ])

    def wrap(request):
        latest_tutorials = Tutorial.objects.recently(40)        # 返回最近40条内容

        return render(request, 'index.html', {
                'latest_tutorials': latest_tutorials,
            })

    if not request.user.is_superuser:           #request.user返回user对象，is_superuser判断是否是超级用户
        wrap = cache_page(900, key_prefix=prefix)(wrap)     #如果是超级用户。这里设置缓存。在这里将ache_page和视图解耦，这里的意思是超级用户不缓存
    return wrap(request)
