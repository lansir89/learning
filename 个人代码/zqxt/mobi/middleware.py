...
#该类是个中间件，用于处理相关的请求头信息，注意该类函数的返回值
class MobileDetectionMiddleware(object):
    ...
    def process_request(request):
        ...
        if "HTTP_ACCEPT" in request.META:                       #如果有响应内容在信息头里
            s = request.META["HTTP_ACCEPT"].lower()             #则将信息头转化为小写
            if 'application/vnd.wap.xhtml+xml' in s:            
                # Then it's a wap browser
                request.mobile = True                           #wap版
                return None

        if "HTTP_USER_AGENT" in request.META:                   #该if用于忽略某些设备，如移动设备等等
