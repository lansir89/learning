...
urlpatterns = patterns('',
    ...
    url(r'^j/(?P<token>.+)/$', 'iclick.views.click_count', name='click-count'),         #统计点击量
    url(r'^robots\.txt$', TemplateView.as_view(
        template_name='robots.txt', content_type='text/plain'), name='robots'),         #robots.txt是一个静态文件
    url(r'^error_tutorial\.py$', 'tutorial.views.error_tutorial', name='error-tutorial'),       #这个不知道是啥
)

# tiy.views里的都是教程里面的简单demo的实现
...
urlpatterns += patterns('',
    ...
    url(r'^sitemap-(?P<section>.+)-(?P<page>\d+)\.xml$',            #sitemap分页
