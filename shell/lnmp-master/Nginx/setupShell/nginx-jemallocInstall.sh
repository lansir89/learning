...
if [ -s pcre-8.37.tar.bz2 ]; then			##如果文件大小非0 
    echo "pcre-8.37.tar.bz2 [found]"
else
    echo "pcre-8.37.tar.bz2 download now..."		#大小为0时下载内容
...
nginxMd5=`md5sum nginx-1.9.5.tar.gz | awk '{print $1}'`			#判断md5
...
mkdir -p /www					#加上-p选项后,系统将自动建立好那些尚不存在的目录,即一次可以建立多个目录
...
mkdir -m 0777 -p /var/log/nginx		#-m后面接权限
mkdir -m 0777 -p /www/logs/nginx

printf "========= pcre install start... =========\n\n"

if [ -s /usr/local/bin/pcregrep ]; then			#pcregrep用于兼容的正则表达式
...
if [ -s /usr/local/lib/libjemalloc.so ]; then
    ...
    if [ -s jemalloc-4.0.3.tar.bz2 ]; then
        ...
        wget http://www.canonware.com/download/jemalloc/jemalloc-4.0.3.tar.bz2				#下载jemalloc，用于内存管理	
    fi
    ...
    isSet=`grep "/usr/local/lib" /etc/ld.so.conf | wc -l`						#判断是否安装成功
    if [ "$isSet" != "1" ]; then
...
cp ../nginx.rcd.txt /etc/rc.d/init.d/nginx			#开机自启动脚本
...
/usr/local/nginx/sbin/nginx -t -c /usr/local/nginx/conf/nginx.conf			#检查配置语法
