...
if [ -s php-5.5.21.tar.bz2 ]; then
    echo "php-5.5.21.tar.bz2 [found]"
else
    echo "php-5.5.21.tar.bz2 download now..."
    wget http://www.php.net/distributions/php-5.5.21.tar.bz2
fi

phpMd5=`md5sum php-5.5.21.tar.bz2 | awk '{print $1}'`							#判断MD5
if [ "$phpMd5" != "8fe631ac52a3df9d6429faad81cf2562" ]; then
    echo "Error: php-5.5.21.tar.bz2 package md5 value is invalid. Please check package download url";
    exit 1
fi

if [ -s php-5.5.21 ]; then
    rm -rf php-5.5.21
fi
tar jxvf php-5.5.21.tar.bz2
