...
#下载并安装gettext库，gettext 是GNU国际化与本地化(i18n)函数库。它常被用于编写多语言程序。
...
#下载并安装libiconv，libiconv是个字符集转换库
